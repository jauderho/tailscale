...
// Complete blocks until an operation completes, then returns true if it was a success.
// Otherwise, it returns returns false, with a possible error.
func (wr *WaitableResult) Complete() (bool, error) {
	wr.cond.L.Lock()
...
func (wr *WaitableResult) Current() (time.Time, bool, error) {
	wr.cond.L.Lock()
	defer wr.cond.L.Unlock()
